...
import { ArrowLeft, Calendar, MapPin, Share2, MessageSquare } from "lucide-react";
...
export default function DetailKegiatanPage() {
  ...
  const kegiatan = kegiatanData.find((item) => item.id === kegiatanId) || kegiatanData[0];
      ...
      <div className="max-w-[1100px] mx-auto w-full px-6 pt-8">
        ...
        <div className="relative w-full aspect-[21/9] md:aspect-[24/10] bg-slate-900 rounded-2xl overflow-hidden mb-8 shadow-sm">
          ...
          <div className="absolute top-6 left-6 z-20">
            <Link href="/kegiatan" className="inline-flex items-center gap-2 bg-[#b4d262] hover:bg-[#9fc53a] text-white px-5 py-2.5 rounded-xl font-bold text-[14px] transition-colors shadow-sm">
        ...
        <div className="mb-10">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col md:flex-row gap-8 shadow-sm">
            <div className="flex items-start gap-4 flex-1">
              ...
              <div>
                <span className="text-[#64748b] text-[12px] font-bold tracking-wider uppercase block mb-1">Waktu & Tanggal</span>
                <span className="text-[#1e293b] text-[15px] font-bold">{kegiatan.timeInfo}</span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 flex-1">
              ...
              <div>
                <span className="text-[#64748b] text-[12px] font-bold tracking-wider uppercase block mb-1">Tempat Kegiatan</span>
                <span className="text-[#1e293b] text-[15px] font-bold">{kegiatan.locationInfo}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="prose prose-slate max-w-none mb-16">
              {kegiatan.content.map((paragraph, index) => (
                <p key={index} className="text-[#4b5563] text-[16px] md:text-[17px] leading-[1.8] mb-6">
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-[22px] font-extrabold text-[#111827]">Komentar</h3>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm mb-10">
                <textarea 
                  ...
                  className="w-full bg-[#F8F9FA] border-none rounded-xl p-4 text-[15px] outline-none min-h-[120px] resize-y mb-4 placeholder:text-slate-400 focus:ring-1 focus:ring-[#99BD4A]/50"
                ></textarea>
                  ...
                  <div key={comment.id} className="flex gap-4">
                    ...
                    <div className="bg-white p-5 rounded-2xl rounded-tl-none border border-slate-100 shadow-sm flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-extrabold text-[#111827] text-[15px]">{comment.name}</span>
                        <span className="text-[#94a3b8] text-[12px] font-medium">{comment.time}</span>
                      </div>
            ...
            <div className="mt-20">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-6 h-1 bg-[#99BD4A] rounded-full"></div>
                <h3 className="text-[22px] font-extrabold text-[#111827]">Kegiatan Lainnya</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {relatedKegiatan.map((item) => (
                  <Link href={`/kegiatan/${item.id}`} key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-slate-100 flex flex-col group">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6 sticky top-24 h-fit">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[17px] font-extrabold text-[#111827]">Postingan Terbaru</h4>
              </div>
              
              <div className="flex flex-col gap-5">
                {recentPosts.map((post) => (
                  <Link href="#" key={post.id} className="flex items-center gap-4 group">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between">
              <span className="font-extrabold text-[#111827] text-[15px]">Bagikan Kegiatan:</span>
